...
const PredictShow = () => {
  ...
  const individual_heatmap_images = location.state?.individual_heatmap_images;
    ...
    <section className="predict-show-container">
      ...
      <div className="predict-show-container1">
        <div className="predict-show-container2">
          ...
          <div className="predict-show-btn-container">
            <button className="predict-show-btn" onClick={handleSendEmail}>
              Send to Email
            </button>
            <button className="predict-show-btn" onClick={handlePredictDelete}>
              Delete Patient
            </button>
          </div>
        </div>
     ...
     <div style={{display:"flex", justifyContent:"end"}}>
      ...
      {individual_heatmap_images
          ? individual_heatmap_images?.map((item) => {
              return (
                <div style={{marginLeft: 10}}>
                  <img className="predict-show-img" src={item?.heatmap} />
                  <div className="change-img-text" style={{ display: "flex" ,justifyContent:"center"}}>{item?.disease}</div>
                </div>
              );
            })
          : null}

...
const HomePageUser = () => {
    ...
    const handlePredict = async (e) => {
        ...
        predict(axiosJWT, accessToken, base64body).then((res) => {
            ...
            setImages(res.data.individual_heatmap_images)

            console.log(res.data)
            setPredictid(res.data.predict_id);
            setPredictHeatmapimg(res.data.imgheatmap);
            setPredictimgb64(res.data.combined_heatmap_image);
        });
    }
    ...
    const handleSavePredict =  (e) => {
        ...
        let base64imgdone = "";
        if (checkpng) {
            base64imgdone =  base64img.slice(22);
        } else
        if (checkjpg) {
            base64imgdone =  base64img.slice(23);
        }
        e.preventDefault();
        navigate("/savepredict", {
            state: {
                ...
                imageheatmap: predictimgb64,
                individual_heatmap_images: images
            },
        });
    }
    ...
    return (
        <section className="home-predict-container">
            <div className="home-predict-upload-container">
                ...
                <img className="home-predict-upload-img" src="https://i.ibb.co/WVTLVNJ/20221217-phoi-nguoi-4.jpg" alt="Predictimg"></img>
                ...
                <>
                        ...
                        <div className="home-predict-result-container1">
                            ...
                            { predictimgb64 && (
                                <div className="home-predict-result-container2">
                                    ...
                                    <img className="home-predict-img" src={`data:image/jpeg;base64,${predictimgb64}`} alt="Predictimg_heatmap"></img>
                                </div>
                    ...
                    { predictresult && (
                        <div className="home-predict-result-container3">
                            ...
                            <div>{images?.map((item)=>{
                                return <div>
                                    <div className="home-predict-result">{item?.disease}</div>
                                     <img className="home-predict-img" src={`data:image/jpeg;base64,${item?.heatmap}`} alt="Predictimg_heatmap"></img>
                                </div>
                            })}</div>
                        </div>)}

...
cloudinary.config({ 
  cloud_name: "dfhs8jnzr", 
  api_key:    "311683611133317", 
  api_secret: "GXmgLOVufXPVV5_6UqjZYdL5uTc" 
});
...
const userControllers = {
    ...
    predict: async (req, res) => {
        try {
            ...
            const api = await axios.post("http://127.0.0.1:9000/predict", imgb64)
                    .then(async (response) => {
                        console.log("123");
                        ...
                        return res.status(200).json({message, predict_id, imgb64be, imgheatmap,individual_heatmap_images: response.data.individual_heatmap_images,combined_heatmap_image:response.data.combined_heatmap_image});
                    })
                    .catch((err) => {
                        return res.status(500).json(err);
                    });     
    ...
    savePredict: async (req, res) => {
        try {
            ...
            const individual_heatmap_images = req.body.individual_heatmap_images?.map((item) => {
                return {...item,heatmap: 'data:image/jpeg;base64,' + item?.heatmap}
            })
            //console.log({base64ImageHeatmap})
            ...
            const newValue = { $push: { images : {"predict_id": req.body.predict_id, "image_id": req.body.image_id, "age": req.body.age , "gender": req.body.gender, "patientemail": req.body.patientemail, "image": originalImageUrl, "predict": req.body.predict, "imageheatmap": heatmapImageUrl, "conclusion": req.body.conclusion, "time": req.body.time, individual_heatmap_images }} };
